...
function Calc_V_Field(df_G, df, L, ξ, dx, dy, ID_Intruder, Δf)

    ID_Intruder = maximum(df.ID)
    frames = minimum(df.Frame):Δf:maximum(df.Frame)
...
function Calc_V_Field(df_G, df, L, ξ, dx, dy, ID_Intruder, frames)
